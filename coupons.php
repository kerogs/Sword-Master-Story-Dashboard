    ...
    <main>
        ...
        <div class="content coupon">
            ...
            <table class="custom-table">
                ...
                <tbody>
                    <?php
                    $couponListing = getFirstCoupons("./data/couponList.json", 2);

                    foreach ($couponListing as $coupon) {
                        if(couponExists($coupon['name'], "./data/account/" . $_COOKIE['sms3_token'] . "/couponClaim.json")){
                            
                        }
                        $statusClass = $claimed ? 'claimed' : 'code';
                        $expirationClass = $claimed ? 'claimed' : 'expiration';
                        $dataCouponName = $claimed ? '' : 'data-coupon-name="' . $coupon['name'] . '"';

                        echo '<tr ' . $dataCouponName . '>';
                        echo '<td>';
                        if ($coupon['type'] == 'Ruby') {
                        ...
                        echo '<span class="type-' . strtolower($coupon['type']) . '">' . $coupon['description'] . '</span></td><td>';
                        echo '<span class="' . $statusClass . '">[ ' . $coupon['name'] . ' ]</span>';
                        echo '</td>';
                        echo '<td class="' . $expirationClass . '">' . $coupon['expiration'] . '</td>';
                        echo '</tr>';
                    }
                    ?>
                </tbody>
